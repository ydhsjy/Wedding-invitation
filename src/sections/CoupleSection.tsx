          ...
          {wedding.couples.map((person, index) => (
            <MotionReveal key={person.name} delay={index * 0.12}>
              <article className="relative grid min-h-[18rem] overflow-hidden rounded-[1.75rem] bg-[#f4f1ec] p-5 shadow-soft sm:grid-cols-[0.9fr_1.1fr] sm:items-center sm:p-7">
                <Image src={person.background} alt="" fill sizes="(max-width: 768px) 95vw, 760px" className={`scale-105 object-cover opacity-35 blur-[2px] ${index === 0 ? "object-left" : ""}`} />
                ...
                <div className={`relative z-10 mt-6 text-center sm:mt-0 sm:px-3 ${index === 0 ? "sm:order-1 sm:text-left" : "sm:order-2 sm:text-right"}`}>
                  ...
                  <p className={`mx-auto mt-5 max-w-sm text-[15px] leading-[21px] text-clay sm:text-base sm:leading-6 lg:text-[8px] lg:leading-3 ${index === 0 ? "sm:ml-0" : "sm:mr-0"}`}>{person.parents}</p>
                </div>
              </article>
